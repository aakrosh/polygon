...
from polygon.utils.utils import save_model, random_enumerate_smiles
from polygon.vae.vae_misc import CosineAnnealingLRWithRestart, KLAnnealer
from abc import ABC, abstractmethod


class AugmentedSmilesDataset(Dataset):
    """Dataset that applies SMILES augmentation on-the-fly.

    Each time a molecule is accessed, a new random SMILES representation
    is generated by shuffling atom indices and selecting a random starting atom.
    This helps the model learn that different SMILES orderings represent the
    same molecule.
    """
    ...
    def __init__(self, smiles_list, augment=True):
        ...
        Args:
            ...
            augment: If True, apply random SMILES enumeration. If False, return original.
        """
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            return random_enumerate_smiles(smiles)
        return smiles
...
class VAETrainer(ABC):
    def __init__(self, model, **kwargs):
        ...
        self.lr_n_mult = 1
        self.lr_end = 0.0003
        self.smiles_augmentation = False
    ...
    def get_dataloader(self, data, batch_size=None, collate_fn=None, shuffle=True, augment=None):
        """Create a DataLoader for training or validation.

        Args:
            data: List of SMILES strings
            batch_size: Batch size (default: self.n_batch)
            collate_fn: Collate function (default: from model)
            shuffle: Whether to shuffle data
            augment: Whether to apply SMILES augmentation. If None, uses self.smiles_augmentation.
                     Set to False explicitly for validation data.
        """
        ...
        # Wrap data in AugmentedSmilesDataset if augmentation is enabled
        if augment is None:
            augment = self.smiles_augmentation

        if augment:
            logging.info("SMILES augmentation enabled for this dataloader")
            dataset = AugmentedSmilesDataset(data, augment=True)
        else:
            dataset = data
    ...
    def fit(self, train_data, val_data=None, n_epoch=None, batch_size=None, save_frequency=None):
        ...
        # Training data uses augmentation if enabled
        train_loader = self.get_dataloader(train_data,
            batch_size=batch_size,
            shuffle=True,
            augment=self.smiles_augmentation)

        # Validation data should never be augmented (need consistent evaluation)
